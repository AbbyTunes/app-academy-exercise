...
// 	if (typeof directories !== 'object') return false;
// 	if (directories[targetFile] === null)  return true ;

// 	return Object.keys(directories).some(directory => {
// 		return fileFinder(directories[directory], targetFile);
// 	});
// }

function fileFinder(directories, targetFile) {
	return Object.entries(directories).some(ele => {
		if (ele[0].startsWith('/')) {
			return fileFinder(ele[1], targetFile);
		} else {
			return ele[0] === targetFile;
		}
	});
}

...
function balancedParens(str) {
	...
	for (let i = 0; i < str.length; i++) {
		let char = str[i];
		pairChar = parenPairObj[char]
		if (pairChar) {
			stack.push(pairChar);
		} else if (char === ")" || char === "]" || char === "}") {
			let deleteChar = stack.pop();
			if (deleteChar !== char) return false;
		}
	}
...
// 		"{": "}"
// 	};
// 	console.log(parenPairObj.keys);
// 	console.log(parenPairObj.values);

// 	if (!str.length) return true;
// 	for (let i = 0; i < str.length; i++) {
// 		let char = str[i];
// 		if (parenPairObj.keys.includes(char)) {
// 			console.log(parenPairObj.keys);
// 			parenStack.push(char);
// 		} else if (parenPairObj.values.includes(char)) {
// 			deleteChar = parenStack.pop();
